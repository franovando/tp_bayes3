...
# Modelo 1: Regresión Cuadrática Bayesiana con priors uniformes
...
-   $y_i$: valor observado de la posición vertical final de la pelota
    después de un tiempo $y_i$ (en metros).
...
En este modelo se utilizaron **priors no informativos**, permitiendo que
los datos tengan un rol principal en la actualización de la distribución a
posteriori. Por lo que, los priors utilizados son:

-   Para $\beta_0$, $\beta_1$ y $\beta_2$: priors uniformes, el plano se encuentra sobre los reales.
...
ggplot() +
  ...
  geom_line(data = curva, aes(x = t_f, y = x_f, group = i), alpha = 0.4, color = "lightblue3") +
...
ggplot(summary_priori, aes(x = t_grilla)) +
  geom_ribbon(aes(ymin = q05, ymax = q95), fill = "lightblue", alpha = 0.4) +
  geom_line(aes(y = mean), color = "blue", linewidth = 1) +
  labs(title = "Distribución predictiva a priori de y",
       x = "t", y = "y_pred") +
  theme_minimal()
...
ggplot() +
  geom_point(data = datos_planetax, aes(x = t, y = x_t), color = "black", alpha = 0.7, size = 2) +
  geom_line(data = curva2, aes(x = t_f, y = x_f, group = i), alpha = 0.7, color = "lightblue") +
  geom_line(data = curva_media2, aes(x = t_f, y = x_f), color = "blue", linewidth = 1) +
  theme_bw() +
  labs(x = "Tiempo (en segundos)", y = "Altura (en metros)")
...
yp_qts2 <- t(apply(y_pred_2, 2, function(x) quantile(x, c(0.025, 0.975))))
y_pred_2 <- matrix(y_pred_2,ncol = 1)

predicciones2 <- data.frame(t=rep(datos_planetax$t, each = 4000),
                           y_pred_2)
...
gravedad <- data.frame(
  modelo = rep(c("Modelo 1","Modelo 2"), each = 4000),
  a = c(a_mod1,a_mod2)
)
ggplot(gravedad) +
  geom_histogram(
...
mu_qts_10.1 <- t(apply(mu_matrix_10.1, 2, function(x) quantile(x, c(0.025, 0.975))))
...
mu_qts_10.3 <- t(apply(mu_matrix_10.3, 2, function(x) quantile(x, c(0.025, 0.975))))
...
mu_qts_10.4 <- t(apply(mu_matrix_10.4, 2, function(x) quantile(x, c(0.025, 0.975))))
