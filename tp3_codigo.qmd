...
Se observa que el posterior de $a$ (la aceleración constante que experimenta la pelota) presenta una forma aproximadamente simétrica, concentrada en torno a un valor cercano a $6m/s^2$. Esto sugiere que la intensidad de la gravedad en el Planeta X es inferior a la terrestre ($\approx 9.8 m/s^2$), pero mayor a la de Marte ($\approx 3.7 m/s^2$).
...
El lanzamiento de la pelota fue realizado por una persona entrenada de manera exigente, que desarrolló la capacidad de lanzar la pelota siempre con la misma velocidad inicial (aproximadamente 8 metros por segundo), por lo que $\beta_1$ se asume normal centrado en 8 y se le asigna un desvío relativamente chico confiando en el entrenamiento realizado.
...
Cada panel de la @fig-7 muestra el histograma de las posiciones simuladas (ỹ) en seis instantes concretos. Inicialmente, las densidades se concentran alrededor de un valor próximo a la altura de lanzamiento, con distribuciones relativamente estrechas. A medida que transcurre el tiempo, los histogramas se ensanchan y se desplazan: primero hacia valores máximos positivos (fase ascendente) y luego hacia valores negativos (fase de caída). Esta evolución ilustra cómo la variabilidad inducida por los priors se traduce en mayor dispersión en etapas avanzadas del vuelo, dado que si se observan los ejes verticales de cada gráfico se observa que la altura de las barras va disminuyendo considerablemente.
...
La distribución parámetro $a$ se encuentra centrada en torno a 6 m/s². La dispersión es relativamente baja (entre 5 y 7), lo cual indica una creencia informativa pero no restrictiva.

La distribución de $\sigma$ es asimétrica, concentrada principalmente entre 0.3 y 1 m, aunque con una cola hacia la derecha que admite errores de hasta aproximadamente 1.5 m. Esta forma refleja una creencia moderada en que los errores son pequeños, pero permite ocasionalmente errores mayores.

El prior para la velocidad inicial se centra alrededor de 8m/s, con una distribución simétrica y estrecha (entre 6 y 10 m/s). Esto sugiere que se tiene una idea relativamente precisa del rango de velocidades plausibles en este tipo de lanzamiento.
...
La curva mostrada en la @fig-9 representa la posición esperada de la pelota a lo largo del tiempo, según el modelo ajustado. Se observa que está es decreciente, lo cual es consistente con el comportamiento físico de un objeto lanzado hacia arriba y afectado por la gravedad. Además, puede notarse que la mayoría de los puntos observados se encuentran contenidos dentro de las bandas de credibilidad del 95%, lo cual indica un buen ajuste del modelo a los datos empíricos.


Con el objetivo de evaluar el comportamiento esperado de la posición de la pelota a lo largo del tiempo en lanzamientos futuros, a continuación se presenta el gráfico correspondiente a la distribución predictiva a posteriori para distintos valores del tiempo.
...
La @fig-10 muestra la distribución predictiva a posteriori de la posición de la pelota en el tiempo, según el modelo 2 ajustado. En este se visualiza la media de las predicciones (en azul), acompañada por las bandas de credibilidad del 95% para la distribución condicional de $\mu$ (en gris oscuro), y para las observaciones futuras (en gris claro).
...
Además, la comparación entre la banda de credibilidad para $\mu$ y la banda de predicción deja en evidencia cómo la incertidumbre aumenta al considerar la variabilidad inherente a nuevas observaciones. En conjunto, este análisis respalda la solidez del modelo 2 tanto en términos de ajuste como de capacidad predictiva.

# Comparación de ambos modelos ajustados 
Con el objetivo de analizar cómo varía la estimación del parámetro de gravedad bajo distintos supuestos previos, se comparan las distribuciones a posteriori obtenidas para dicho parámetro en dos modelos bayesianos diferentes. El Modelo 1 utiliza un prior uniforme, mientras que el Modelo 2 incorpora un prior más informativo. 

El siguiente gráfico ilustra el posterior del parámetro asociado a la gravedad mediante boxplots, facilitando la comparación visual entre ambos modelos.
...
```{r}
#| label: fig-12
#| fig-cap: "Distribución de la gravedad para ambas velocidades iniciales y entre \n los dos modelos" 
#| fig-pos: "H"
...
g6ms.1 <- t_caida_6ms.1 %>% ggplot() +
  ...
  labs(title = "Modelo 1 - Velocidad inicial: 6 m/s",
...
geom_histogram(
  ...
  labs(title = "Modelo 2 - Velocidad inicial: 6 m/s",
       x = "Tiempo (segundos)", y = "Densidad") +
  theme_bw()
...
geom_histogram(
  ...
  labs(title = "Modelo 1 - Velocidad inicial: 12 m/s",
       x = "Tiempo (segundos)", y = "Densidad") +
  theme_bw()
...
geom_histogram(
  ...
  labs(title = "Modelo 2 - Velocidad inicial: 12 m/s",
       x = "Tiempo (segundos)", y = "Densidad") +
  theme_bw()
...
Al analizar las distribuciones a posteriori del tiempo de caída estimado para dos velocidades iniciales bajo dos modelos diferentes, se observan diferencias claras tanto en la ubicación como en la dispersión de las predicciones.
